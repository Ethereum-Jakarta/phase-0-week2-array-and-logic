...
function angkaPalindrome(num) {
  ...
  let angka_terbalik = num.toString().split('').reverse('').join('');

  if(num < 10){
      return num+1;
  }
  else if(num === Number(angka_terbalik)){
      return num;
  }
  else{
      for (let x = 10; x <= 9999; x++){
          angka_terbalik2 = x.toString().split('').reverse('').join('');
          if (x === Number(angka_terbalik2)){
              if (num < x){
                  return x;
                  break;
              }
