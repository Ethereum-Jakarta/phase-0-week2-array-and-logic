...
function mengelompokkanAngka(arr) {
  ...
  let resultArrayMultiDimention = [[], [], []];

  for(let i = 0; i < arr.length; i++) {
    if(arr[i] % 3 === 0) {
        resultArrayMultiDimention[2].push(arr[i]);
    } else if(arr[i] % 2 === 0) {
        resultArrayMultiDimention[0].push(arr[i]);
    } else {
        resultArrayMultiDimention[1].push(arr[i]);
    }
  }
  return resultArrayMultiDimention;
}
