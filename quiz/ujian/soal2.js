...
function mengelompokkanAngka(arr) {
  ...
  let kelompok_genap = [];
  let kelompok_ganjil = [];
  let kelompok_kelipatan_tiga = [];
  
  for (let i = 0; i < arr.length; i++) {

    if (arr[i] % 3 === 0) {
        kelompok_kelipatan_tiga.push(arr[i]);
        continue;
    } 
    if (arr[i] % 2 === 1) {
        kelompok_ganjil.push(arr[i]);
        continue;
    } 
    if (arr[i] % 2 === 0) {
        kelompok_genap.push(arr[i]);
        continue;
    } 
  }
  
  return [kelompok_genap, kelompok_ganjil, kelompok_kelipatan_tiga];
}
