...
function perkalianUnik(arr) {
  let total = arr.reduce((accumulator,current) => accumulator * current, 1) 
...
function tentukanDeretAritmatika(arr) {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    const selisihsaatini = arr[i + 1] - arr[i]
    const selisihpembanding = arr[1] - arr[0] 

    if (selisihsaatini !== selisihpembanding) {
      return false;
    }
  }
  return true;
...
function tentukanDeretGeometri(arr) {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    ...
    const ratiopembanding = arr[1] / arr[0]

    if (ratio !== ratiopembanding) {
      return false;
    }
  } return true;
}
